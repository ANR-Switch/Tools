...
global {
	...
	bool use_google_map_data <- false;
	...
	map filtering <- ["building"::[], "shop"::[], "historic"::[], "amenity"::[], "sport"::[], "military"::[], "leisure"::[], "office"::[],
		"waterway"::[], "leisure"::[], "natural"::[], "highway"::[]
	];
	...
	init {
		...
		save road crs:"EPSG:3857" to:dataset_path +"/roads.shp" type: shp attributes: ["type"::type];
		save waterway crs:"EPSG:3857" to:dataset_path +"/waterways.shp" type: shp attributes: ["type"::type];
		save leisure crs:"EPSG:3857" to:dataset_path +"/leisures.shp" type: shp attributes: ["type"::type];
		save natural crs:"EPSG:3857" to:dataset_path +"/naturals.shp" type: shp attributes: ["type"::type];
	...
	action load_OSM_data {
		osm_file osmfile;
			...
			//create the building agents from the data
			create road from: geom with:[type:: get("highway")];
			create waterway from: geom with:[type:: get("waterway")];
			create leisure from: geom with:[type:: get("leisure")];
			create natural from: geom with:[type:: get("natural")];
			
			// Remove agents with empty shape
			ask road + waterway + leisure + natural {
				if (shape = nil) {do die;} 
			}
			
			// Remove roads that are not polylines
			ask road + waterway {
